...
import type { CreateSessionResponse, HealthResponse, LogoutResponse, UserResponse } from "../api/types";
import { apiRequest } from "../api/client";
import {
  useCreateSessionMutation,
  useLogoutMutation,
  useProtectedStatusQuery,
  useUserQuery,
} from "../api/hooks";
...
describe("api hooks", () => {
  ...
  it("fetches protected status when token exists", async () => {
    const response: HealthResponse = { status: "success", message: "ok" };
  ...
  it("fetches current user when token exists", async () => {
    const response: UserResponse = { email: "user@example.com" };
    apiRequestMock.mockResolvedValueOnce(response);
  ...
  it("logs out with token", async () => {
    const response: LogoutResponse = { status: "success", message: "ok" };
    apiRequestMock.mockResolvedValueOnce(response);
